...
CUDAStream<T>::CUDAStream(const unsigned int ARRAY_SIZE, const int device_index)
{
  ...
  // Set device
  int count;
  cudaGetDeviceCount(&count);   // device enumeration, 3.2.6.1
  check_error();
  if (device_index >= count)
    throw std::runtime_error("Invalid device index");
  cudaSetDevice(device_index);  // device selection, 3.2.6.2
  check_error();
  ...
  sums = (T*)malloc(sizeof(T) * DOT_NUM_BLOCKS);   // #define DOT_NUM_BLOCKS 256
